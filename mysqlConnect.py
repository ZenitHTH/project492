...
def InsertRTD(datekey,score,time,dmy,prikey):
    if datakey == None or score == None or time == None or dmy == None:
        print("ERROR")
    else : 
        ...
        sql = "INSERT INTO RealTimeData(score,time,dmy,datekey,prikey) VALUES({score},\"{time}\",\"{dmy}\",{datekey},\"{prikey}\");".format(
            score=score,time=time,dmy=dmy,datekey=datekey,prikey=prikey)
        cursor.execute(sql)
        db.commit()
...
def InsertTD(datakey,score,dmy):
    if datakey == None or score == None or dmy == None:
        ...
        sql = "INSERT INTO TimeData(datekey,scorepermin,dmy) VALUES({datakey},{scorepermin},\"{dmy}\");".format(
            datekey=datakey,scorepermin=score,dmy=dmy)
        cursor.execute(sql)
        db.commit()
